...
function init(go)
    game_object = go
    character_controller = gameobject_get_component(game_object, "character_controller")
end

function update(delta)

    right_dir.x, right_dir.y, right_dir.z = vec3_rotate(global_right.x, global_right.y, global_right.z, yaw, 0, 1, 0)
    forward_dir.x, forward_dir.y, forward_dir.z = vec3_rotate(global_forward.x, global_forward.y, global_forward.z, yaw,
        0, 1, 0)
    ...
    position.x, position.y, position.z = transform_get_position(game_object)
    ...
    cam_forward.x, cam_forward.y, cam_forward.z = vec3_rotate(global_forward.x, global_forward.y, global_forward.z,
        pitch, 1, 0, 0)
